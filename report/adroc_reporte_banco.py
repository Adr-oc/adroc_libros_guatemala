...
class AdrocReporteBanco(models.AbstractModel):
    _name = 'report.adroc_libros_guatemala.adroc_reporte_banco'
    ...
    def lineas(self, datos):
        ...
        for linea in self.env['account.move.line'].search([
            ...
            totales['debito'] += linea.debit
            totales['credito'] += linea.credit
            ...
            # Si la cuenta no tiene moneda o la moneda de la cuenta es la misma de la compania
            ...
            # Sino, Si la cuenta si tienen moneda y la moneda de la cuenta es diferente que la de la compania
    ...
    def balance_inicial(self, datos):
        self.env.cr.execute(
            ...
            'coalesce(sum(amount_currency), 0) as balance_moneda '
            'from account_move_line where account_id = %s and date < %s',
